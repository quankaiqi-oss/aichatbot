...
def build_summary_pdf(summary: dict) -> bytes:
    ...
    with PdfPages(buffer) as pdf:
        ...
        ax.add_patch(plt.Rectangle((0, 0.82), 1, 0.18, color="#5C4444", transform=ax.transAxes))
        ax.add_patch(plt.Rectangle((0.08, 0.82), 0.012, 0.18, color="#B4CFCB", transform=ax.transAxes))

        fig.text(0.11, 0.92, "ShopCare MY", fontsize=24, weight="bold", color="#FFFFFF")
        fig.text(0.11, 0.885, "Customer Support Chat Summary", fontsize=13, color="#EDE7D5")
        fig.text(
            0.11,
            0.855,
        ...
        fig.text(0.11, 0.72, "Conversation Outcome", fontsize=15, weight="bold", color="#5C4444")
        fig.text(
            0.11,
            0.695,
        ...
        for index, detail in enumerate(summary["conversation_details"], start=1):
            ...
            if row_bottom < 0.12:
                fig.text(
                    0.11,
                    0.06,
                    "This PDF is generated by ShopCare MY to help users keep a simple support reference.",
                    fontsize=9.2,
                    color="#756464",
                )
                fig.text(0.78, 0.06, "ShopCare MY", fontsize=9.2, weight="bold", color="#5C4444")
                pdf.savefig(fig)
                plt.close(fig)
                ...
                ax.add_patch(plt.Rectangle((0, 0.88), 1, 0.12, color="#5C4444", transform=ax.transAxes))
                ax.add_patch(plt.Rectangle((0.08, 0.88), 0.012, 0.12, color="#B4CFCB", transform=ax.transAxes))
                fig.text(0.11, 0.94, "Conversation Details", fontsize=19, weight="bold", color="#FFFFFF")
                fig.text(0.11, 0.91, "Continued support chat records", fontsize=10.5, color="#EDE7D5")
